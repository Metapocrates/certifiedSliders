...
export default function SubmitResultPage() {
  ...
  const [state, formAction] = useFormState(submitResultAction, initialState);
  ...
  return (
    <main className="mx-auto max-w-xl p-6">
      ...
      <p className="text-sm text-gray-500 mb-6">
        Enter your mark as <code>SS.ss</code> or <code>M:SS.ss</code>. Wind is optional for sprints/jumps (e.g., <code>2.0</code> or <code>-1.3</code>). Times are normalized server-side and adjusted in the DB.
      </p>

      {state?.ok === false && (
        <div className="mb-4 rounded border border-red-300 bg-red-50 p-3 text-sm text-red-700">
          {("error" in state && state.error) || "Submission failed."}
        </div>
      )}
      {state?.ok === true && (
      ...
      <form ref={formRef} action={formAction} className="space-y-4">
        <div>
          <label className="block text-sm font-medium mb-1" htmlFor="event">Event</label>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input id="mark" name="mark" required placeholder="53.76 or 1:53.21" className="w-full rounded border p-2" />
          </div>
          <div>
        ...
        <div className="pt-2">
          <button type="submit" className="rounded-xl px-4 py-2 border shadow-sm hover:shadow">Submit</button>
        </div>
      </form>
    </main>
  );
