...
import { supabaseServer } from "@/lib/supabase/server"; // we'll add this next
...
export default async function AthletePage({ params }: { params: { username: string } }) {
  ...
  // 2) Fetch PRs + Results by athlete_id (no athlete_username column needed)
    ...
    <div className="container mx-auto max-w-6xl px-4 py-6">
      ...
      <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
        ...
        {typeof profile.star_rating === "number" ? (
          <div className="self-start rounded-2xl border px-3 py-2 text-sm shadow-sm">
            ⭐ {profile.star_rating.toFixed(1)} / 5.0
          </div>
        ) : null}
      </div>
